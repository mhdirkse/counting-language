...
class SampleMultipleStatementHandler extends AbstractExpressionHandler
implements StatementSource, TerminalFilterCallback {
    private final TerminalFilter terminalFilter;
    private SampleMultipleStatement statement;
    ...
    SampleMultipleStatementHandler(final int line, final int column) {
        statement = new SampleMultipleStatement(line, column);
    ...
    public void addExpression(final ExpressionNode expression) {
        if(statement.getNumSampled() == null) {
        	statement.setNumSampled(expression);
        } else {
